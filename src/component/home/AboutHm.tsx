...
export default function AboutTrueSunNew() {
    return (
        <section className="relative overflow-hidden bg-gray-50 pt-16 pb-20 sm:pt-20 sm:pb-24">
            ...
            <div className="absolute inset-0 bg-[url('/img/solar-pattern.svg')] opacity-5 [background-size:200px]" aria-hidden="true" />

            <div className="relative mx-auto max-w-7xl px-6 lg:px-8">

                {/* === SECTION 1: CENTERED HEADLINE & VALUE PROPOSITION === */}
                <div className="mx-auto max-w-7xl text-center">
                    <span className="inline-flex items-center gap-2 rounded-full bg-orange-400/90 px-4 py-2 text-sm font-semibold text-black ring-1 ring-amber-300">
                ...
                <div className="mt-16 grid gap-8 md:grid-cols-4">
                    ...
                    <PillarCard
                        ...
                        features={["Project Management Consulting (PMC)", "Tech Selection & Audits", "Detailed Payback Modeling"]}
                    />
                    <PillarCard
                        Icon={DollarSign}
                        ...
                        features={["Carbon Footprinting & Roadmaps", "Carbon Credit Value Chain Advisory", "Optimal Finance Structuring"]}
                    />
                    <PillarCard
                        Icon={Factory}
                        title="End-to-End Rooftop EPC"
                ...
                <div className="mt-20 border-1 px-2 border-gray-600 py-5 rounded-xl shadow-xl">
                    <div className="grid lg:grid-cols-3 gap-10">
                        ...
                        <div className="lg:col-span-1 flex items-center justify-end">
                            <a
                                href="#contact"
                                className="group relative inline-flex items-center justify-center rounded-xl px-8 py-4 text-lg font-bold text-white transition duration-300 transform hover:scale-[1.02]"
                            >
                                <span className="absolute inset-0 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 shadow-[0_10px_35px_rgba(255,153,0,0.4)]" />
...
function PillarCard({ Icon, title, desc, features }: { Icon: React.ElementType, title: string, desc: string, features: string[] }) {
    return (
        <div className="group flex flex-col rounded-xl border border-gray-900/50 bg-orange-100/15 hover:bg-orange-500/25 p-6 text-center shadow-xl transition duration-300 hover:shadow-2xl hover:border-black hover:scale-105">
            ...
            <p className="mt-3 flex-grow text-gray-600">{desc}</p>

            {/* List of services/features */}
            <ul className="mt-5 space-y-2 text-sm text-gray-700 text-left border-t border-gray-100 pt-4">
                {features.map((feature, index) => (
                    <li key={index} className="flex items-start">
                        <svg className="h-5 w-5 flex-shrink-0 text-amber-500 mr-2 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
